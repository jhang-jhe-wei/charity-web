...
module NotifiesHelper
  def cover(auth_link)
    bubble do
      ...
      body do
        text "LINE Notify 是一個「訊息推播」服務，點擊下方的設定說明，你將了解到如何設定 LINE Notify，設定完成後，你將可以訂閱右邊\b列表的各式通知。", wrap: true
      end
  ...
  def page(title, content)
    bubble do
      ...
      footer paddingTop: '0px' do
        separator color: '#c2c2c2', margin: 'xl'
        message_button '取消訂閱', "取消通知/#{title}"
        message_button '開始訂閱', "訂閱通知/#{title}", style: 'primary', color: '#F3D23C'
      end
    end
  end
end
